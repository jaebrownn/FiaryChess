...
public class MoveValidationChecksGUI {
	...
	public static void initializeBoard() {
		while (true) {
			...
			for (int i = 0; i < 10; i++) {
				for (int j = 0; j < 10; j++) {
					...
					if (boardPlacement[i][j] == 'p' || boardPlacement[i][j] == 'P') {
						...
						Bishop current = new Bishop(i, j, boardPlacement[i][j],
								Character.isLowerCase(boardPlacement[i][j]), free);
						board.setPiece(current, i, j);
						...
						Knight current = new Knight(i, j, boardPlacement[i][j],
								Character.isLowerCase(boardPlacement[i][j]), free);
						board.setPiece(current, i, j);
						...
						Queen current = new Queen(i, j, boardPlacement[i][j],
								Character.isLowerCase(boardPlacement[i][j]), free);
						board.setPiece(current, i, j);
						...
						Amazon current = new Amazon(i, j, boardPlacement[i][j],
								Character.isLowerCase(boardPlacement[i][j]), free);
						board.setPiece(current, i, j);
					} else {
						Piece current = new Piece(i, j, boardPlacement[i][j],
								Character.isLowerCase(boardPlacement[i][j]), free);
						;
						board.setPiece(current, i, j);
					}
			...
			if (JOptionPane.showConfirmDialog(null, "Do you want to read the moves from a move file?",
					"Chose your input method!", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				// yes option
				moveFileInput = true;

				while (true) {
					try {
						moveFile = new File(JOptionPane.showInputDialog("Enter the path of the move file:"));
						scAllMoves = new Scanner(moveFile);
						break;
					} catch (FileNotFoundException e) {
						JOptionPane.showMessageDialog(null, "Illegal move file, please try again.");
						continue;
					}
				}
			} else {
			...
			StdDraw.setCanvasSize(1650, 900);
			StdDraw.setPenColor(StdDraw.GRAY);
			StdDraw.setXscale(-5, 17);
			...
			StdDraw.text(-2.5, 11.7, "Piece allocations:");
			Scanner scPieces = new Scanner(pieces);
			double printRow = 11;
			while (scPieces.hasNext()) {
				String currentPc = scPieces.nextLine();
				System.out.println("*** "+currentPc+" ***");
				Scanner scCurrent = new Scanner(currentPc).useDelimiter(":");
				StdDraw.picture(-3.5, printRow, board.getIcon(scCurrent.next().toUpperCase().charAt(0)), 1, 1);
				StdDraw.text(-1.5, printRow, scCurrent.next() + "");
				printRow = printRow - 1.1;
			}
			...
			boolean moveFileHasNext = true;

			if (moveFileInput) {
				moveFileHasNext = scAllMoves.hasNextLine();
			} else {
				moveFileHasNext = true;
			}
			
			while (!checkMate && moveFileHasNext) {
				
				String currentMove = "";

				if (moveFileInput) {
					currentMove = scAllMoves.nextLine();
					if (currentMove.charAt(0) == '%') {
						continue;
					}
				}
				...
				if (moveFileInput) {
					while (enterPressed == false) {
						if (StdDraw.isKeyPressed(KeyEvent.VK_ENTER)) {
							...
							if (currentMove.contains("0-0")) {
								...
								src = arr[0];
								srcCol = arr[0].charAt(0) - 'a';
								srcRow = 10 - Integer.parseInt(arr[0].substring(1));

								dst = arr[1];
								dstCol = arr[1].charAt(0) - 'a';
								dstRow = 10 - Integer.parseInt(arr[1].substring(1));
								source = board.getPiece(srcRow, srcCol);
								destination = board.getPiece(dstRow, dstCol);
							}
							enterPressed = true;
							break;
						} else {
					...
					while (countClicks < 2) {
						if (StdDraw.isMousePressed() && countClicks == 0) {
							...
							if (src.equals("invalid")) {
								continue;
							} else if (src.contains("0-0")) {
								currentMove = src;
								break;
							} else if ((('a' > (int) (src.charAt(0)) || 'j' < (int) (src.charAt(0)))
									|| (1 > Integer.parseInt(src.substring(1))
											|| 10 < Integer.parseInt(src.substring(1))))) {
								JOptionPane.showMessageDialog(null, "Invalid cell! Please re-enter.");
								continue;
							}
							StdDraw.show();
							StdDraw.pause(200);
							...
							if (dst.equals("invalid")) {
								continue;
							} else if (dst.contains("0-0")) {
								JOptionPane.showMessageDialog(null,
										"Invalid option! Please complete move which has already been started.");
								continue;
							}

							else if ((('a' > (int) (dst.charAt(0)) || 'j' < (int) (dst.charAt(0)))
									|| (1 > Integer.parseInt(dst.substring(1))
											|| 10 < Integer.parseInt(dst.substring(1))))) {
								JOptionPane.showMessageDialog(null, "Invalid cell! Please re-enter.");
								continue;
							}
							StdDraw.show();
					...
					if (currentMove.contains("0-0")) {
						...
						while (invalid) {
							...
							} else if (1 > Integer.parseInt(src.substring(1))
									|| 10 < Integer.parseInt(src.substring(1))) {

							} else if (1 > Integer.parseInt(dst.substring(1))
									|| 10 < Integer.parseInt(dst.substring(1))) {
				 ...
				 *************************************************************************************************/
				if (currentMove.contains("0-0")) {
					System.out.println("Performing castling move with: " + currentMove);
					...
					System.out.println("Performing normal move with: " + currentMove);
					if ((source instanceof Pawn || source instanceof DrunkenSoldier) && !moveFileInput) {
						// alternative for if: board.getPiece(dstRow, dstCol) instanceof Pawn ||
						// board.getPiece(dstRow, dstCol) instanceof DrunkenSoldier && (dstRow == 9 ||
						// dstRow == 0)
						...
						if (promoting) {
						String promotionChar = JOptionPane.showInputDialog(
								"Please enter the character of the piece you'd like to promote to: ");
						
						if (player) {
							promotionChar = promotionChar.toLowerCase();
						} else {
							promotionChar = promotionChar.toUpperCase();
						}
						
						currentMove = currentMove + promotionChar;
						}
					...
					if (currentMove.contains("x")) { // capture, should move to a slot that is not free
						if (board.getPiece(dstRow, dstCol).isFree()
								|| board.getPiece(dstRow, dstCol).isBlack() == player) {
							// illegal capture
							JOptionPane.showMessageDialog(null,
									"Illegal capture! Destination cell must have opponent players piece.");
							continue;
						}
					...
					if (!board.getPiece(srcRow, srcCol).validMove(srcRow, srcCol, dstRow, dstCol, board)) {
						if (currentMove.contains("x")) { // capture
							// illegal capture
							if (!((board.getPiece(srcRow, srcCol) instanceof Pawn
									&& ((Pawn) (board.getPiece(srcRow, srcCol))).validCapture(srcRow, srcCol, dstRow,
											dstCol, board))
									|| (board.getPiece(srcRow, srcCol) instanceof DrunkenSoldier
											&& ((DrunkenSoldier) (board.getPiece(srcRow, srcCol))).validCapture(srcRow,
													srcCol, dstRow, dstCol, board)))) {
								JOptionPane.showMessageDialog(null,
										"Illegal capture! This piece cannot move to the desired cell.");
								continue;
							}
						} else { // move
							// illegal move
							JOptionPane.showMessageDialog(null,
									"Illegal move! This piece cannot move to the desired cell.");
							continue;
						}
					}
					...
					if (promotion) { // g9-g10=Q+
						System.out.println((promoteTo + "").toLowerCase().charAt(0));
						if (!(officerIndexes.indexOf((promoteTo + "").toLowerCase().charAt(0)) >= 0)) {
							System.out.println("1");
							JOptionPane.showMessageDialog(null, "Illegal promotion!");
							continue;
						} else if ((pieceNums[pieceIndexes.indexOf((promoteTo + "").toLowerCase().charAt(0))] == 0
								|| promoteTo == 'E' || promoteTo == 'e')) {
							System.out.println("");
							JOptionPane.showMessageDialog(null, "Illegal promotion!");
							continue;
						}
					...
					whiteInCheckBefore = whiteCheck;
					blackInCheckBefore = blackCheck;
					checkPlayerInCheck(temp);
			...
			printGUI();
			JOptionPane.showMessageDialog(null, "Finished reading move file");
	...
	private static void printGUI() {
		...
		StdDraw.filledRectangle(14, 9.5, 2, 0.5);
		StdDraw.setPenColor(StdDraw.BLACK);
		...
		StdDraw.text(14, 9.5, curPlayer + "'s turn");

		StdDraw.setPenColor(StdDraw.WHITE);
		StdDraw.filledRectangle(14, 7.5, 2.25, 0.75);
		StdDraw.filledRectangle(14, 4.5, 2.25, 1.25);
		StdDraw.filledRectangle(14, 1.5, 2.25, 0.75);

		StdDraw.setPenColor(StdDraw.BLACK);
		Font font2 = new Font(Font.MONOSPACED, Font.BOLD, 18);
		Font font3 = new Font(Font.MONOSPACED, Font.BOLD, 60);
		StdDraw.setFont(font2);

		StdDraw.text(14, 10.5, "Click here to enter text for a move");
		StdDraw.text(14, 8.5, "Half move clock:");
		StdDraw.text(14, 6, "Castling opportunities:");
		StdDraw.text(14, 2.5, "Move counter:");

		StdDraw.setFont(font);
		StdDraw.setPenColor();
		StdDraw.text(14, 7.5, halfmoveClock + "");
		
		StdDraw.setFont(font2);
		StdDraw.text(12.5, 5, "BQ");
		StdDraw.text(13.5, 5, "BK");
		StdDraw.text(14.5, 5, "WQ");
		StdDraw.text(15.5, 5, "WK");
		
		StdDraw.setFont(font3);
		StdDraw.text(12.5, 4.5, castlingOpps.charAt(0) + "");
		StdDraw.text(13.5, 4.5, castlingOpps.charAt(1) + "");
		StdDraw.text(14.5, 4.5, castlingOpps.charAt(2) + "");
		StdDraw.text(15.5, 4.5, castlingOpps.charAt(3) + "");

		StdDraw.setFont(font);
		StdDraw.text(14, 1.5, moveCounter + "");

		StdDraw.rectangle(14, 10.5, 2.85, 0.5);
		StdDraw.rectangle(14, 7.5, 2.25, 0.75);
		StdDraw.rectangle(14, 4.5, 2.25, 1.25);
		StdDraw.rectangle(14, 1.5, 2.25, 0.75);
	...
	private static void checkPlayerInCheck(Board temp) {
		...
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				...
				if (!(temp.getPiece(i, j).isFree() || temp.getPiece(i, j) instanceof King) && checkPossible) {
					if (posPlayer) {
						whiteCheck = true;
					} else {
						blackCheck = true;
	...
	private static void checkPlayerInCheckMoveFile() {
		...
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				...
				if (!(board.getPiece(i, j).isFree() || board.getPiece(i, j) instanceof King) && checkPossible) {
					if (posPlayer) {
						whiteCheck = true;
					} else {
						blackCheck = true;
	...
	public static String getPieceFromClick(String srcdst) {
		...
		while (true) {
			if ((x >= 1 && x <= 10) && (y <= 10 && y >= 1)) {
				...
				StdDraw.pause(300);
				break;
			} else if ((x >= 11 && x <= 17) && (y >= 10 && y <= 11)) {
				StdDraw.setPenColor(StdDraw.GREEN);
				piece = JOptionPane.showInputDialog("Enter the  " + srcdst + " coordinates:");
				StdDraw.setPenColor(StdDraw.GREEN);
				System.out.println("Drawing square at postion: " + (piece.charAt(0) - 'a' + 1) + " "
						+ Integer.parseInt(piece.substring(1)));
				if (!(('a' > (int) (piece.charAt(0)) || 'j' < (int) (piece.charAt(0)))
						|| (1 > Integer.parseInt(piece.substring(1)) || 10 < Integer.parseInt(piece.substring(1))))) {
					StdDraw.square(piece.charAt(0) - 'a' + 1, Integer.parseInt(piece.substring(1)), 0.5);
				}
				break;
			} else if ((x >= 11 && x <= 17) && (y >= 3 && y <= 6)) {
				String playerText = "white";
				if (player) {
					playerText = "black";
				}

				Object[] options = { "Kingside", "Queenside" };

				if (JOptionPane.showOptionDialog(null, "Which castling opportunity would you like to use?", "Castling",
